...
**Telegram** (`whisperfast/telegram/`) слухає в тому самому процесі FTW: галочка біля іконки або кнопка у вікні налаштувань. Якщо галочка була увімкнена перед закриттям, слухач стартує знову (`telegram_listener_enabled`). Режим `account` читає особисті чати і названі групи через Telethon; режим `bot` — через локальний telegram-bot-api. Щойно файл прийнято, рядок пишеться в лог FTW, ще до завантаження. У режимі акаунта те саме стартове повідомлення йде в Обране. Посилання YouTube, Instagram і Facebook за замовчуванням лише скачуються і повертаються в чат; у чергу Whisper вони йдуть, коли увімкнено `telegram_social_to_queue`. Режим навчання (`telegram_learn_mode`) не обробляє новий чат, доки користувач не вирішить, брати файл чи ні. Кілька файлів або посилань з одного чату за паузу близько 3 с (не довше 10 с) збираються в одне питання. «Ні» записує чат у список пропуску. Закриття вікна або 10 с без відповіді пропускає лише цю пачку і не ставить її в чергу; рядок логу лишається клікабельним. `telegram_intake` (`all`, `media`, `links`) обмежує, що слухач бере, і для навчання, і для чатів без питання. Після обробки в той самий чат повертаються TXT, файли AI, MP3 з відео і відрізок аудіо. Стовпець Telegram у черзі надсилає той самий набір; якщо чату немає, вікно питає назву групи або контакту (10 останніх імен — `.ftw_tg_recent.json`). Зупинка черги не надсилає файл, розшифровку якого обірвано; уже завершені файли і їхній AI відправляються самі. Той самий файл (стабільний id Telegram, зокрема пересилання) не завантажується вдруге: запис лежить у `.ftw_tg_seen.json`. Якщо розшифровка чи файли AI вже є, у новий чат ідуть вони; якщо файл ще в черзі, новий чат отримає результат разом із першим. Після успішної обробки джерело переноситься до папки результатів і лишається на диску; у чат надсилається копія. Зв’язок «файл → чат» лежить у `.ftw_tg_origin.json` і переписується, коли `source_relocate` переносить джерело. Поки слухач працює, вікно FTW має лишатися відкритим.

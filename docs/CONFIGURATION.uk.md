...
| `telegram_learn_mode` | `false` | Нові чати не обробляються, доки користувач не відповість, брати файл чи ні. «Ні» записує чат у список пропуску. Закриття вікна або 10 с без відповіді пропускає лише цей файл. Кілька файлів з одного чату за кілька секунд питаються одним питанням. Рядок у логу відкриває питання знову. |
| `telegram_intake` | `"all"` | Що брати з чату: `all` (голос, відео і посилання), `media` (лише голос і відео) або `links` (лише посилання). |

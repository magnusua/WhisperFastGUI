...
class CopilotProvider:
    ...
    def process(
        ...
        if not self.has_api_credentials(credentials):
            return run_browser_fallback(self.id, COPILOT_BROWSER_URL, txt_path, prompts, log_func)

        return run_provider_chain(
            self.id,
            lambda user_msg: call_azure_chat(endpoint, api_key, deployment, api_version, user_msg),
            txt_path,
            prompts,
            log_func,
            on_file_created,
